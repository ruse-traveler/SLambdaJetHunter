...
void QuickTreePlotter() {
  ...
  // i/o parameters
  const TString sOutput  = "lambdaJetTreePlots_charge.d15m2y2024.root";
  const TString sInput   = "output/lambdaJetTree_charge.pp200py8jet10run8.d15m2y2024.root";
  const TString sInTree = "LambdaJetTree";

  // cuts to apply and labels
  const vector<TString> vecCutsToApply = {
    "",
    "EvtNLambdas>=1",
    "(EvtNLambdas>=1)&&(LambdaJetID>-1)&&(LambdaZ<2)"
  };
  const vector<TString> vecCutLabels = {
    "NoCuts",
    "IsLambdaEvt",
    "LambdaHasAssociatedJet"
  };
